# -*- coding: utf-8 -*-
"""
Created on Wed Apr 14 15:09:32 2021
...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 13 15:34:37 2021
...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 13 14:17:54 2021
...
parser.add_argument("--model", type=str, default='punet1')
parser.add_argument('--log_dir', default='logs/test_layers_shuffle', help='Log dir [default: logs/test_log]')
...
def sample_task(TotalBatch, Batchsize, task_num):
    # generate total task dataset
    input_data = 2*torch.rand(TotalBatch, Batchsize, task_num, 2)
    #gt_data = input_data
    return input_data
 
# def sample_task(TotalBatch, task_num):
#     # generate total task dataset
#     input_data = 2*torch.rand(TotalBatch, task_num, 2)
#     #gt_data = input_data
#     return input_data   
...
def nonuniform_sampling(num, sample_num):
    ...
    while len(sample) < sample_num:
        a = int(np.random.normal(loc=loc, scale=0.3) * num)  # loc: mean scale: std var
        if a < 0 or a >= num:
            continue
        sample.add(a)
    return list(sample)
if __name__ == '__main__':
    ...
    model.train()
    train_dataset = sample_task(100, 4, 2048)
    for epoch in range(args.max_epoch):
        loss_list = []
        emd_loss_list = []
        rep_loss_list = []
        
        index = [k for k in range(len(train_dataset))]
        random.shuffle(index)
        train_dataset_shuffle = train_dataset[index]
                
        #for batch in train_loader:
        for i, batch in enumerate(train_dataset_shuffle):
            if i % 20 == 0:
                print(i)
            optimizer.zero_grad()
            ...
            input_data  = batch
            gt_data = input_data
            radius_data = torch.ones(len(input_data)).unsqueeze(1)
            
            sample_idx = nonuniform_sampling(input_data.shape[1], sample_num=1024)
            input_data = input_data[:,sample_idx, :]
              
            
            gt_data = gt_data.float().cuda().contiguous()
            input_data = input_data.float().cuda()
            #gt_data = gt_data[..., :2].contiguous() #3
            radius_data = radius_data.float().cuda()

            preds = model(input_data)
            #emd_loss, rep_loss = loss_func(gt_data, gt_data, radius_data)
            #print('emd_loss', emd_loss)
            #emd_loss, _, _ = sinkhorn(gt_data, gt_data)
            #print('emd_loss1', torch.mean(emd_loss))
            emd_loss, _, _ = sinkhorn(preds, gt_data)
            emd_loss = torch.mean(emd_loss)
            #print('emd_loss2', emd_loss)
            _, rep_loss = loss_func(preds, gt_data, radius_data)
            loss = 100*emd_loss + rep_loss
            #print('rep_loss', rep_loss)
            #print('loss',loss)
            #print('loss',emd_loss + rep_loss)
            
            loss_value = loss.item()
            emd_lossvalue = emd_loss.item()
            rep_lossvalue = rep_loss.item()
            
            loss.backward()
            optimizer.step()
            loss_list.append(loss_value)
            emd_loss_list.append(emd_lossvalue)
            rep_loss_list.append(rep_lossvalue)
