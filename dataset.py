...
class PUNET_Dataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print('Get_input_data',input_data.shape)
        gt_data = self.gt[index]
        #print('Get_gt_data',gt_data.shape)
        radius_data = np.array([self.radius[index]])
        #print('self.data_npoint',self.data_npoint)
        #print('radius_data.shape0', radius_data.shape)

        sample_idx = utils.nonuniform_sampling(self.data_npoint, sample_num=self.npoint)
        input_data = input_data[sample_idx, :]
        #print('1Get_input_data',input_data.shape)
        ...
        if self.use_norm:
            ...
            #print('type',type(radius_data))
            #print('radius_data.shape', radius_data.shape)
