...
class _PointnetSAModuleBase(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.npoint = None    #新要插值的点，new_xyz的个数        
        self.groupers = None
        self.mlps = None
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, npoint=None, new_xyz=None) -> (torch.Tensor, torch.Tensor):
        ...
        if new_xyz is None:
            new_xyz = pointnet2_utils.gather_operation(
                xyz_flipped,
                pointnet2_utils.furthest_point_sample(xyz, self.npoint)  # FPS抽样方法，进行下采样
            ).transpose(1, 2).contiguous() if self.npoint is not None else None  # output: (B, C, npoint)  转置后 -> (B, npoint, C)
        ...
        for i in range(len(self.groupers)):
            ...
            #print('new_features1.shape', new_features.shape)
